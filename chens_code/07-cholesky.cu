...
#include <cuda/experimental/__stf/frequency/GpuFrequencyTuner.cuh>
...
void DPOTRF(cublasFillMode_t uplo, class matrix<double>& A, int A_row, int A_col)
{
  ...
  // auto t = ctx.task(exec_place::device(A.get_preferred_devid(A_row, A_col)), Akk.rw(), potrf_buffer.write(), devInfo.write());
  auto t = ctx.task(exec_place::device_auto(), Akk.rw(), potrf_buffer.write(), devInfo.write());
  t.set_symbol("DPOTRF");
...
{
  nvtx_range r("GEMM");
  auto t = ctx.task(exec_place::device_auto(),
...
void PDPOTRF(matrix<double>& A)
{
...
#ifdef FREQTUNER
  int primary_device_id = 1;
  GpuFrequencyTuner tuner(primary_device_id);
  
  // char command[256];
  // snprintf(command, sizeof(command), "nvidia-smi -i %d -pm 1 > /dev/null 2>&1", primary_device_id);
  // system(command);

  // tuner.setFrequency(KernelType::DGEMM);
#endif
  ...
  std::future<void> freq_future; // To track async frequency changes

  for (int K = 0; K < NBLOCKS; K++)
  {
    // Launch frequency change asynchronously for POTRF
    // cuda_safe_call(cudaSetDevice(A.get_preferred_devid(K, K)));

#ifdef FREQTUNER
    freq_future = tuner.setFrequencyAsync(KernelType::DPOTRF);
    
    // While frequency is changing in another thread, the previous iteration's
    // data transfers are happening on the GPU
    
    // Wait for frequency change to complete before enqueueing this kernel
    GpuFrequencyTuner::waitForFrequencyChange(freq_future);
  #endif
    // Now enqueue the POTRF task with the correct frequency already set
    DPOTRF(CUBLAS_FILL_MODE_LOWER, A, K, K);

    for (int row = K + 1; row < NBLOCKS; row++)
    {
      // Async frequency change for DTRSM
      // cuda_safe_call(cudaSetDevice(A.get_preferred_devid(row, K)));
#ifdef FREQTUNER
      freq_future = tuner.setFrequencyAsync(KernelType::DTRSM);
      
      // Wait for frequency, then enqueue task
      GpuFrequencyTuner::waitForFrequencyChange(freq_future);
#endif
      ...
      {
        // Async frequency change for DGEMM
        // cuda_safe_call(cudaSetDevice(A.get_preferred_devid(row, col)));
#ifdef FREQTUNER
        freq_future = tuner.setFrequencyAsync(KernelType::DGEMM);
      ...
      // Async frequency change for SYRK
      // cuda_safe_call(cudaSetDevice(A.get_preferred_devid(row, row)));
#ifdef FREQTUNER
      freq_future = tuner.setFrequencyAsync(KernelType::DSYRK);
...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < ndevs; i++) {
      ...
      for (int j = 0; j < ndevs; j++) {
          if (i == j) continue;
          cuda_safe_call(cudaDeviceCanAccessPeer(&canAccessPeer, i, j));
          printf("P2P %d -> %d: %s\n", i, j, canAccessPeer ? "YES" : "NO");
          
          if (canAccessPeer) {
              cudaError_t err = cudaDeviceEnablePeerAccess(j, 0);
              if (err == cudaSuccess) {
                  printf("  Enabled P2P access %d -> %d\n", i, j);
              } else if (err == cudaErrorPeerAccessAlreadyEnabled) {
                  // This is fine, clear the error
                  cudaGetLastError();
                  printf("  P2P access %d -> %d already enabled\n", i, j);
              } else {
                  // Some other error occurred
                  printf("  Failed to enable P2P access %d -> %d: %s\n", 
  ...
  // DISABLE ALL P2P - force host staging
  // printf("Disabling P2P access on all GPUs (broken P2P topology)...\n");
  // for (int i = 0; i < ndevs; i++) {
  //     cuda_safe_call(cudaSetDevice(i));
  //     for (int j = 0; j < ndevs; j++) {
  //         if (i != j) {
  ...
  // }
  // cuda_safe_call(cudaSetDevice(0));
  // printf("=== ALL P2P DISABLED - USING HOST STAGING ===\n");
  ...
  std::cout << "KERNEL_START_TIMESTAMP:" << start << std::endl;
  ...
  std::cout << "KERNEL_END_TIMESTAMP:" << end << std::endl;
