...
void tiledLUPart(bool verify, bool dot)
{
    ...
    {
        if (verbose) std::cout << "Generating slice " << i << std::endl;
        ...
        if (i < sliceCount)
        {
            ...
            for (int sliceRow = T - lowerBlocks; sliceRow < T; sliceRow++)
            {
                ...
                checkCudaErrors(cudaMemcpy2D(
                    getMatrixBlock(d_matrix, i, sliceRow), sizeof(double) * local_width, d_block,
                    sizeof(double) * B, sizeof(double) * B, B, cudaMemcpyHostToDevice));
                // TODO: take a look here!

                if (sliceRow != sliceCol)
                {
                    checkCudaErrors(cublasDgeam(cublasHandle, CUBLAS_OP_T, CUBLAS_OP_T, B, B, &one,
                                                d_block, B, &zero, d_block, B, d_block_trans, B));

                    kernel_nvshmem_put2D_multiBlock<<<108, 1024>>>(
                        getMatrixBlock(d_matrix, sliceRow / nPE, sliceCol), local_width,
                        d_block_trans, B, B, sliceRow % nPE);
                    nvshmem_quiet();
                }
            }
        }
        ...
        if (verify)
        {
            if (verbose) std::cout << "Collecting slice(s) " << i << std::endl;
            checkCudaErrors(cudaMemcpy2D(getMatrixBlock(originalMatrix.get(), i * nPE + myPE, 0, N),
                                         sizeof(double) * N, getMatrixBlock(d_matrix, i, 0),
                                         sizeof(double) * local_width, sizeof(double) * B, N,
            ...
            for (int dstPE = 1; dstPE < nPE; dstPE++)
            {
                if (i * nPE + dstPE < T)
                {
                    kernel_nvshmem_get2D_slice_multiBlock<<<108, 1024>>>(
                        getMatrixBlock(d_buffer, dstPE, 0), buffer_width,
                        getMatrixBlock(d_matrix, i, 0), local_width, B, N, dstPE);
                    nvshmem_quiet();
                    checkCudaErrors(cudaDeviceSynchronize());
                    checkCudaErrors(
                        cudaMemcpy2D(getMatrixBlock(originalMatrix.get(), i * nPE + dstPE, 0, N),
                                     sizeof(double) * N, getMatrixBlock(d_buffer, dstPE, 0),
                                     sizeof(double) * buffer_width, sizeof(double) * B, N,
                                     cudaMemcpyDeviceToHost));
                }
            }
        }
    ...
    for (int i = 0; i < totalNodes; i++) h_dependencies[i] = 0;

    cudaGraph_t graph;
    checkCudaErrors(cudaGraphCreate(&graph, 0));
    auto tiledLUGraphCreator =
        std::make_unique<mustard::TiledGraphCreator>(s, graph, false, totalNodes);
    ...
    {
        if (verbose)
            std::cout << "CUDA Graph generation progress " << float(k) / float(T) * 100.0 << "%"
                      << std::endl;
        //* A[k][k] = GETRF(A[k][k])
        //* L[k][k]*U[k][k] = A[k][k]
        ...
        if (activePE == myPE)
        {
            tiledLUGraphCreator->beginCaptureOperation(std::make_pair(k, k),
                                                       {std::make_pair(k, k)});
            checkCudaErrors(cusolverDnDgetrf(cusolverDnHandle, B, B,
                                             getMatrixBlock(d_matrix, local_k, k),
                                             local_width,  // ?: N or local_width
                                             d_workspace_cusolver, NULL, d_info));
            ...
            if (myPE == peToPrint && nodeIndex == nodeToPrint)
                kernel_print_submatrix<<<108, 1024, 0, s>>>(getMatrixBlock(d_matrix, local_k, k), B,
                                                            B, local_width, nodeIndex);
        ...
        else if (distance < (T - k))
        {
            ...
            // Chain broadcast: wait for dependency, forward to neighbor if needed, then get remote
            // data.
            ...
            kernel_nvshmem_get2D<<<1, 1024, 0, s>>>(
                getMatrixBlock(d_buffer, k % nPE, k, buffer_width), buffer_width,
                getMatrixBlock(d_matrix, local_k, k), local_width, B, k % nPE);
        ...
        for (int i = k + 1; i < T; i++)
        {
            ...
            if (activePE == myPE)
            {
                ...
                checkCudaErrors(cublasDtrsm(cublasHandle,
                                            ...
                                            B, B, &one, getMatrixBlock(d_matrix, local_k, k),
                                            local_width, getMatrixBlock(d_matrix, local_k, i),
                                            local_width));

                if (myPE == peToPrint && nodeIndex == nodeToPrint)
                    kernel_print_submatrix<<<108, 1024, 0, s>>>(
                        getMatrixBlock(d_matrix, local_k, i), B, B, local_width, nodeIndex);
            ...
            {  // if there are neighbor tiles
                ...
                if ((k % nPE) != neighborPE && distance + 1 < (T - k))
                    mustard::kernel_dep_update_noq<<<1, 1, 0, s>>>(d_dependencies, nodeIndex,
                                                                   neighborPE, myPE);
                kernel_nvshmem_get2D<<<1, 1024, 0, s>>>(
                    getMatrixBlock(d_buffer, k % nPE, i, buffer_width), buffer_width,
                    getMatrixBlock(d_matrix, local_k, i), local_width, B, k % nPE);
                nvshmemx_quiet_on_stream(s);
                tiledLUGraphCreator->endCaptureOperation();
            }
            nodeIndex++;
            ...
            if (activePE == myPE)
            {
                ...
                {  // this should only be copied if the parent is remote
                    checkCudaErrors(cublasDtrsm(
                        cublasHandle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N,
                        CUBLAS_DIAG_NON_UNIT, B, B, &one,
                        getMatrixBlock(d_buffer, k % nPE, k, buffer_width), buffer_width,
                ...
                else
                {
                    checkCudaErrors(cublasDtrsm(
                        cublasHandle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N,
                        CUBLAS_DIAG_NON_UNIT, B, B, &one, getMatrixBlock(d_matrix, local_k, k),
                        local_width,                                          // k + k * N;
                ...
                {
                    kernel_print_submatrix<<<108, 1024, 0, s>>>(
                        getMatrixBlock(d_matrix, local_i, k), B, B, local_width, nodeIndex);
                }
                tiledLUGraphCreator->endCaptureOperation();
            }
            nodeIndex++;
            ...
            for (int j = k + 1; j < T; j++)
            {
                ...
                if (activePE == myPE)
                {
                    ...
                    checkCudaErrors(cublasSetWorkspace(
                        cublasHandle, d_workspace_cublas[myNodeIndex++], cublasWorkspaceSize));
                    tiledLUGraphCreator->beginCaptureOperation(
                        std::make_pair(i, j),
                    ...
                    {  // this should only be copied if the parent is remote
                        checkCudaErrors(cublasGemmEx(
                            cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N, B, B, B, &minusOne,
                            getMatrixBlock(d_matrix, local_i, k), CUDA_R_64F, local_width,
                            getMatrixBlock(d_buffer, k % nPE, j, buffer_width), CUDA_R_64F,
                            buffer_width, &one, getMatrixBlock(d_matrix, local_i, j), CUDA_R_64F,
                            local_width, CUBLAS_COMPUTE_64F, CUBLAS_GEMM_DEFAULT));
                    }
                    else
                    {
                        checkCudaErrors(cublasGemmEx(
                            cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N, B, B, B, &minusOne,
                            getMatrixBlock(d_matrix, local_i, k), CUDA_R_64F, local_width,
                            getMatrixBlock(d_matrix, local_k, j), CUDA_R_64F, local_width, &one,
                    ...
                    {
                        kernel_print_submatrix<<<108, 1024, 0, s>>>(
                            getMatrixBlock(d_matrix, local_i, j), B, B, local_width, nodeIndex);
                    }
                    tiledLUGraphCreator->endCaptureOperation();
                }
                nodeIndex++;
    ...
    if (verify)
    {
        ...
        for (int iPE = 1; iPE < nPE; iPE++)
        {
            ...
            if (verbose)
                std::cout << "Collect " << remoteSliceCount << " slices from PE " << iPE
                          << std::endl;
...
int main(int argc, char **argv)
{
    ...
    if (verbose) printf("Hello from NVSHMEM_PE=%d/%d\n", myPE, nPE);

    B = N / T;

    if (myPE == 0)
    {
        std::cout << "PARTITIONED LU with N=" << N << " (" << T << " of " << B << "x" << B
                  << " tiles)" << std::endl;
    ...
    if (nPE < 2)
    {
        std::cerr << "Partitioned LU requires multiple GPUs. Run with multiple MPI processes."
                  << std::endl;
        return 1;
    }
